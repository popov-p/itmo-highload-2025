...
### 1. Постановка задачи
#### 1.1. Общее описание архитектуры
- Пусть имеется большая майнинг-ферма на 1000 видеокарт. 
- Пользователю требуется производить мониторинг актуального состояния серверной, основываясь на информации, отправляемых с датчиков контроля и получать alerts о состоянии системы. 
- Датчик контроля - небольшое программируемой устройство, считывающее информацию с отдельно взятой видеокарты о:
  1. Температуре графического ядра.
  2. Температуре памяти.
  3. Количестве потребляемой видеопамяти.
  4. Загрузке ГП в %.
  5. Временной отметке снятия показаний с устройства.
- Датчик поддерживает отправку сообщений по протоколу HTTP по REST API.
- При помощи REST API датчик должен отправлять информацию о состоянии отдельно взятой видеокарты при помощи пакета унифицированного формата.
#### 1.2 Уточнения к постановке задачи
...
![usecase-diagram](../plots/AHLA-1/AHLA-1-usecase.png)
